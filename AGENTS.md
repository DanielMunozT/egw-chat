# AGENTS.md — EGW Research (OpenAI Codex / Agents)
...
1. **Search** using `scripts/search.py` with `--json` flag for structured output
2. **Parse** the JSON results to extract text and reference codes
3. **Cite** using reference codes (e.g., "GC 299.1") from metadata
4. **Cross-reference** by running multiple queries with related terms
5. **Read full books** from `books/<lang>/` for expanded context
6. Use `--lang` to search non-English corpora
